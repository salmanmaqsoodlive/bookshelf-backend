...
const createBook = asyncHandler(async (req, res) => {
    try {
        ...
        const book = await Book.create({ ...req.body, publicationYear, user: userId })
        res.status(201).send({ success: true, message: "Book created successfully", book })
    } catch (error) {
        res.status(500)
...
const updateBook = asyncHandler(async (req, res) => {
    try {
        ...
        res.status(200).send({ success: true, message: "Book updated successfully", book: updatedBook })
    } catch (error) {
        res.status(500)
        throw new Error(error.message)
    }
...
const getBooks = asyncHandler(async (req, res) => {
    try {
        ...
        let books = await Book.find(query).sort(sort);
